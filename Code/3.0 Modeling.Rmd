...
```{r setup, include=FALSE}
git_model_data<- GET('https://raw.githubusercontent.com/mchang674/ADS599_Capstone/refs/heads/main/Data%20Files/model_data.csv')
model_data <- readr::read_csv(git_model_data)
...
near_to_pov<- model_data %>%
  select(!pov_ind) %>%
  mutate(edu_cat = as.factor(edu_cat))
...
lm2<- lm(POVPIP ~ edu_cat + dis + edu_cat:dis, data = train2, weights = PWGTP)
summary(lm2)

# educational attainment with disability interactions and standard hours worked per week
lm3<- lm(POVPIP ~ edu_cat + dis + WKHP, data = train2, weights = PWGTP)
summary(lm3)

# educational attainment with disability, standard hours worked per week, and geographical information
lm4<- lm(POVPIP ~ age + edu_cat + tract_in + dis + tract_in:dis, data = train2, weights = PWGTP)
summary(lm4)
```
...
test2_Results <- data.frame(
  obs = test2$POVPIP,
  lm1 = predict(lm1, test2[, 3:37]),
  lm2 = predict(lm2, test2[, 3:37]),
  lm3 = predict(lm3, test2[, 3:37]),
  lm4 = predict(lm4, test2[, 3:37])
)

RMSE(test2_Results$obs, test2_Results$lm1)
RMSE(test2_Results$obs, test2_Results$lm2)
RMSE(test2_Results$obs, test2_Results$lm3)
RMSE(test2_Results$obs, test2_Results$lm4)
```
