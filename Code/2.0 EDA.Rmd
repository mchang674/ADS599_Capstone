...
```{r setup, include=FALSE}
git_pums_data <- GET('https://raw.githubusercontent.com/mchang674/ADS599_Capstone/refs/heads/main/Data%20Files/pums_data.csv') 

git_puma_data<- GET('https://raw.githubusercontent.com/mchang674/ADS599_Capstone/refs/heads/main/Data%20Files/2020_Census_Tract_to_2020_PUMA.txt')


pums_data<- readr::read_csv(git_pums_data)
puma<- readr::read_csv(git_puma_data)
```
...
pums_data_col<- pums_data %>%
  mutate(edu_cat = case_when(SCHL == '01' | SCHL =='02' | SCHL =='03'| SCHL =='04'| SCHL =='05'| SCHL =='06'| SCHL =='07'| SCHL =='08' ~1,
                                SCHL == '09'| SCHL =='10'| SCHL =='11'~2,
                                SCHL == '12'| SCHL =='13'| SCHL =='14'| SCHL =='15'~3,
                                SCHL == '16'| SCHL =='17'~4,
                                SCHL == '18'| SCHL =='19'~5,
                                SCHL == '20'| SCHL =='21'~6,
                                SCHL == '22'| SCHL =='23'| SCHL =='24'~7),
...
tract_per_puma<- puma %>%
  filter(STATEFP == '17') %>% 
  group_by(PUMA5CE) %>%
  summarise(tract_in = length(unique(TRACTCE))) 


pums_data_col<- pums_data_col %>%
  inner_join(tract_per_puma, by = join_by(PUMA == PUMA5CE))
...
With a disability           0.229
Without a disability        0.903
...
```{r}
pums_data_col %>%
  select(AGEP, DDRS, DEAR, DEYE, DOUT, DPHY, DREM, SSIP, SSP, WAGP, PINCP, tract_in, edu_cat, POVPIP) %>%
  cor() %>%
  corrplot(diag = FALSE, type = 'upper')
```
...
model_data<- pums_data_col %>%
  select(PWGTP, POVPIP, AGEP, edu_cat, COW_desc, DIS, RACAIAN, RACASN, RACBLK, RACNH, 
         ...
         DEYE, DOUT, DPHY, DRAT, DRATX, DREM, tract_in) %>%
