...
## For individuals near or below the poverty line, can a model be built to see where they fall in the poverty percentage?
...
```{r}
lin_model <- lm(POVPIP~., data = train2[2:42], weights = train2$PWGTP)

summary(lin_model)

test2$pred <- predict(lin_model, test2)

RMSE(test2$POVPIP, test2$pred)
```

```{r}
test2 %>%
  mutate(dis = as.factor(dis),
         resid = POVPIP - pred) %>%
  ggplot() +
  geom_point(aes(x = POVPIP, y = resid, color = dis))
```
