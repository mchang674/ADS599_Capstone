...
county<- readr::read_csv(here::here("Ruralurbancontinuumcodes2023.csv")) %>%
  filter(Attribute == 'RUCC_2023' & State == 'IL')

puma<- readr::read_csv(here::here("2020_Census_Tract_to_2020_PUMA.txt"))
```
...
```{r}
pums_data_col %>%
  select(AGEP, DDRS, DEAR, DEYE, DOUT, DPHY, DREM, SSIP, SSP, WAGP, PINCP, tract_in, POVPIP) %>%
  cor() %>%
  corrplot(diag = FALSE, type = 'upper')
```
...
```{r}
# Save my own copy
write_csv(model_data, here::here("model_data_matt.csv"))

# Load mine
model_data_matt <- read_csv(here::here("model_data_matt.csv"))
