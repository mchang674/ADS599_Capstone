...
## MATT's Additions start - 

##Poverty and Education additions

#poverty indicators
```{r}
pums_data_col <- pums_data_col %>%
  mutate(below_poverty = as.integer(POVPIP < 100),
         deep_poverty = as.integer(POVPIP < 50)
         )
```

#poverty table education & disability - weighted
```{r}
...
#Map of education and disability related to Poverty (<100% of threshold)
...
#Map of education and disability related to Deep Poverty (<50% of threshold)
```{r}
...
#Line plot for poverty rates by education - disabled vs. not
```{r}
...
```{r}
table(pums_data$ST, useNA = "ifany")
table(pums_data$STATEFIP, useNA = "ifany")
...
trainingRows <- createDataPartition(model_data$POVPIP, p = .80, list = FALSE) 
