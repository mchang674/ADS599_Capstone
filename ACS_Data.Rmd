...
pums_data<- get_pums(variables = c('AGEP', 'PUMA'), variables_filter = list(AGEP = 20:65), year = 2023, state = "IL", survey = "acs5", recode = TRUE)

pums_data<- pums_data %>%
  filter(substr(SERIALNO, 1, 6) == '2023HU') %>%
  select(SERIALNO, SPORDER, AGEP, WGTP, PWGTP, PUMA)

for (var in vars){
  var_data<- get_pums(variables = c(var, 'PUMA'), variables_filter = list(AGEP = 20:65), year = 2023, state = "IL", survey = "acs5", recode = TRUE) %>%
    filter(substr(SERIALNO, 1, 6) == '2023HU') %>%
    select(SERIALNO, SPORDER, starts_with(var), PUMA)
  
  pums_data<- pums_data %>%
    left_join(var_data, by = join_by(SERIALNO, SPORDER, PUMA))
