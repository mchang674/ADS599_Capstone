...
80: health insurance re-code
96: total person's income
98: income to poverty ratio re-code
107: race
190: self employment allocation

```{r}
vars<- c(pums_var$var_code[4:14], pums_var$var_code[20:26], pums_var$var_code[59], pums_var$var_code[61:65], pums_var$var_code[77], pums_var$var_code[96], pums_var$var_code[98],pums_var$var_code[107], pums_var$var_code[190])
...
```{r}
var_data2<- get_pums(variables = 'DIS', variables_filter = list(AGEP = 20:65), year = 2023, state = "IL", survey = "acs5", recode = TRUE) %>%
    filter(substr(SERIALNO, 1, 4) == '2023') %>%
    select(SERIALNO, SPORDER, starts_with('DIS'))

pums_data_2<- pums_data %>%
  left_join(var_data2, by = join_by(SERIALNO, SPORDER))
```


```{r}
write_csv(pums_data_2, '/Users/mtc/ADS/ADS 599/599 Project/pums_data.csv')
```
